...
--Percent
t[#t+1] = LoadFont("_sf rounded pro 50px")..{
	InitCommand=cmd(horizalign,right;addx,220-4;addy,95;zoom,1.5;shadowlength, 1;diffuse,color("#ffffff"));
	OnCommand=function(self)
	local CaloriesToday;
	CaloriesToday = PROFILEMAN:GetProfile(PLAYER_1):GetCaloriesBurnedToday();
	if CaloriesToday >= 0 and CaloriesToday <= 5 then
...
t[#t+1] = LoadFont("_sf rounded pro 50px")..{
	InitCommand=cmd(addx,255-4;addy,95;shadowlength,1;zoom,1.5;diffuse,color("#ffffff");settext,"%");
	};
--pictures
t[#t+1] = Def.Sprite{
